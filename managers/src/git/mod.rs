...
impl GitManager {
    ...
    pub fn get_repo_url(&self) -> Result<String, String> {
        ...
        match remote.url() {
            None => {
                return Err(
                    "Не удалось найти ссылку на репозиторий".to_string()
                );
            }
    ...
    pub fn get_current_branch(&self) -> Result<Branch, String> {
        Ok(Branch::from(
            self.repository
                .head()
                .unwrap()
    ...
    pub fn checkout(
        ...
        match self.get_existed_branch(target_branch.to_string()) {
            ...
            Err(err) => match parent_branch {
                Some(branch) => {
                    Printer::print_warning(
                        format!(
                            "Ветка {} не найдена. Переключаюсь на родительскую ({}) и создаю целевую ветку...",
                    ...
                    let _parent =
                        self.get_existed_branch(branch.to_string())?;
            ...
            .output()
        {
            Ok(output) => {
                ...
                let lines = output
                    .stdout
                    .lines()
                    .find(|l| !l.as_ref().unwrap().starts_with("??"));
